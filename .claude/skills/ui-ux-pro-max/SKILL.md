...
> **BOUND TO CIRCULANT.** In this repo this skill supplies **layout and typography reasoning only**.
> It does **not** get a vote on palette, font family, corner radius, or surface treatment.
>
> | This skill returns | What this repo does with it |
> |---|---|
> | Palette / hex values | **DISCARD.** Use the CIRCULANT tokens in `/CLAUDE.md` § CIRCULANT TOKENS. |
> | Font pairing | **DISCARD.** Space Grotesk only. |
> | Border radius / elevation / gradient / glass | **DISCARD.** Flat UI, sharp 90° corners. |
> | Stack recommendation | **DISCARD.** Vanilla HTML/CSS/JS only — never React/Tailwind/npm. |
> | Spacing scale, grid, hierarchy, fold order, touch targets, contrast, motion timing, a11y, anti-patterns | **KEEP.** This is what the skill is here for. |
>
> Run it with `--stack html-tailwind` for the closest reasoning match, then hand-translate to vanilla CSS.
> **CIRCULANT wins every conflict.** If output disagrees with `/CLAUDE.md`, `/CLAUDE.md` is right.
