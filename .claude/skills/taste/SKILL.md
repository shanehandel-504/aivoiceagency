...
> **BOUND TO CIRCULANT.** In this repo Taste runs as a **subtractive** pass only — step 4 of the ENGINE PIPELINE in `/CLAUDE.md`.
> Its job here is to **strip AI-slop defaults** (generic gradients, badge rows, template card grids, stock-illustration energy,
> centered-hero-over-mesh, Inter + slate-900, infinite decorative loops). It does **not** propose a new palette, a new font,
> a new corner radius, or a new aesthetic family.
>
> - **The Design Read is already written**: dark operator-grade voice-AI landing, Space Grotesk, flat, sharp 90° corners, vast negative space.
> - **Dials are fixed for this repo**: `DESIGN_VARIANCE: 4` · `MOTION_INTENSITY: 3` · `VISUAL_DENSITY: 3`. Restraint over spectacle — these pages sell to contractors at 2AM, not to Awwwards judges.
> - **Section 0.C "ask one question"** — do NOT. Grill-me (pipeline step 1) already resolved intent. Proceed.
> - Any rule below that would change color, type family, radius, or stack: **DISCARD**. `/CLAUDE.md` § CIRCULANT TOKENS wins every conflict.
